...
class DepthData:
    ...
    def __write_data(self):
        # bidsとasksの各tsの5板分をDataFrameに保存
        ...
        df = pd.concat([df_bids, df_asks], axis=1, keys=['bids', 'asks'])
        if self.flg_created_file:
            df.to_csv('Data/'+self.symbol+'_depth.csv', mode='a', header=False)
        else:
            df.to_csv('Data/'+self.symbol+'_depth.csv')
    ...
    def __write_data2(self):
        ...
        df_combined.to_csv(f'Data/depth/apexpro_{self.symbol}_depth.csv', mode=mode, header=header)
        if not self.flg_created_file:
            self.flg_created_file = True


class ApexproDepthData:
    @classmethod
    def initialize(cls):
